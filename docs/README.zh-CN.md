...
查询全部符合条件的会话，最多并发三个请求。运行中和自动等待中的会话可直接查询；
ready 会话只有在 Agent Watch 确认提供方输入框为空后才会查询，needs-input 和 error
会话仍会被拒绝。Agent Watch 还会校验保存的 pane 身份，拒绝向正被其他 tmux client
激活且输入框非空的 pane 发送按键，并对两个提供方发送完全相同的临时 `/btw` 提示。

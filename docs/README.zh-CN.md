...
按 `b` 可向所选的运行中 Codex 或 Claude 会话请求结构化全局进度，按 `B` 可批量查询
全部符合条件的运行中会话，最多并发三个请求。Agent Watch 会校验保存的 pane 身份，
拒绝向正被其他 tmux client 激活的 pane 发送按键，并对两个提供方发送完全相同的临时
`/btw` 提示。非运行中会话不会被查询，避免在审批界面或空闲输入框中意外输入。
...
发起请求；回答会短暂出现在提供方可关闭的旁路浮层中，不会写入 SQLite，也不会进入
远端通知。
